...
The system operates on a three-stage pipeline designed for Accuracy and Auditability:
    ...
    Ingestion (The Blob): Users paste unformatted text—syslogs, firewall outputs, or email headers—directly into a central processing terminal.

    Extraction (Regex Engine): The backend utilizes specific Regular Expression (Regex) profiles to "snip" high-value data points (IPs, Usernames, Timestamps) from the noise.

    Human Verification (The Review): Extracted data is presented in a tabular format. An analyst performs a final manual check, adds qualitative comments, and assigns a status (PASS/FAIL/THREAT) before committing to the database.
